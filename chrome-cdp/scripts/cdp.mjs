...
function formatPageList(pages, activeUrl = null) {
  ...
  return pages.map(p => {
    ...
    const marker = activeUrl && p.url === activeUrl ? '* ' : '  ';
    return `${marker}${id}  ${title}  ${p.url}`;
  }).join('\n');
}
...
async function getActiveChromeUrl() {
  ...
  const browsers = ['Google Chrome', 'Brave Browser', 'Microsoft Edge', 'Chromium'];
...
async function main() {
  ...
  if (cmd === 'list' || cmd === 'ls') {
    ...
    const activeUrl = await getActiveChromeUrl();
    if (activeUrl) {
      console.log('(* = current active tab in Chrome)');
    }
    console.log(formatPageList(pages, activeUrl));
